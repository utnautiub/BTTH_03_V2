<!DOCTYPE html>
   ...
   <body>
      ...
      <div class="menu min-vh-100 p-5 bg-warning">
            <a class="btn btn-success mb-3" href="<?php $_SERVER['DOCUMENT_ROOT'] ?>/CSE485_0923/BTTH_03_V2/app/views/Admin/insertBook.php">Add</a>
            <?php
               if(isset($_GET['mes'])){
         ...
         <table class="table text-center">
               ...
               <tbody>
                  <?php
                     foreach ($data as $row):
                        $id = $row->getId();
                        $bookName = $row->getBookName();
                        $publicationYear = $row->getPublicationYear();
                        $idAuthor = $row->getIdAuthor();
                        $json_token = json_encode(["id" => $id, "bookName" => $bookName, "publicationYear" => $publicationYear, "idAuthor" => $idAuthor]); ?>
                        <tr>
                           <td scope="row"><?= $id ?></td>
                           <td scope="row"><?= $bookName ?></td>
                           <td scope="row"><?= $publicationYear ?></td>
                           <td scope="row"><?= $idAuthor ?></td>
                           <td scope="row">
                              <a href="<?php $_SERVER['DOCUMENT_ROOT'] ?>/CSE485_0923/BTTH_03_V2/app/views/Admin/editBook.php?token=<?= urlencode($json_token); ?>">
                                 ...
                                 <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-sm" role="document">
                                    <div class="modal-content">
                                       ...
                                       <div class="modal-body">
                                             Bạn có chắc chắn muốn xóa sinh viên <?=$bookName?> này không?
                                       </div>
                                       <div class="modal-footer">
                                             <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                                             <a href=
                                                "<?php $_SERVER['DOCUMENT_ROOT'] ?>/CSE485_0923/BTTH_03_V2/public/index.php?controller=Admin&action=DeleteBook&id=<?=$id?>"
